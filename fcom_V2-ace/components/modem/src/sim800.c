...
#define GPIO_OUTPUT_PIN_SIM_SEL ((1ULL<<CONFIG_EXAMPLE_GPIO_MODEM_PWRKEY) | (1ULL<<CONFIG_EXAMPLE_GPIO_MODEM_RESET))
#define GPIO_INPUT_PIN_SIM_SEL (1ULL<<CONFIG_EXAMPLE_GPIO_MODEM_STATUS)
/**
...
static esp_err_t sim800_power_up()
{
    ...
    if (!status) {
        ...
        // Power-on module (pulse of 2s on PWRKEY pin)
...
static esp_err_t sim800_open(modem_dce_t *dce)
{
    ...
    if (!sync) {
        ...
        ESP_LOGD(DCE_TAG, "Start waiting 12 sec (minimal reset time)");
        for (int i = 0; i < 12; i++)
        {
            vTaskDelay(1000 / portTICK_PERIOD_MS);
            ESP_LOGD(DCE_TAG, ".");
...
modem_dce_t *sim800_init(modem_dte_t *dte)
{
    ...
    if (error != ESP_OK)
    {
        ESP_LOGE(DCE_TAG, "error configuring outputs");
    }
    //interrupt of rising edge
    io_conf.intr_type = GPIO_PIN_INTR_DISABLE;
    //bit mask of the pins, use GPIO0 here
    io_conf.pin_bit_mask = GPIO_INPUT_PIN_SIM_SEL;
    //set as input mode
    ...
    if (error != ESP_OK)
    {
        ESP_LOGE(DCE_TAG, "error configuring input");
    }
